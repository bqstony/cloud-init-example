...
Dieses Modul erstellt eine Linux VM und installiert die Demons für einen Azure IOT Edge. Die VM wird anhand eines [cloud-init](https://cloudinit.readthedocs.io/) seed files aufgesetzt. Damit sich der Host verbinden kann, wird ein IOT Hub edge device connectionstring erwartet. 

> Das Deplyomend ist nicht blockierend, es kann also etwas dauern bis die VM bereitsteht.


## Voraussetzung

Diesem Modul muss ein Pfad zu einer Cloud-Init Datei übergeben werden für das installieren des edge-host. Folgend ein Beispiel:
...
write_files:
  ...
  - path: /srv/certs/${certfilename}.openssl.cnf
    content: |
      [ req ]
      default_bits= 4096
      ...
      [ alt_names ]
      URI.1 = urn:${certfilename}:host.docker.internal:
      DNS.1 = ${certfilename}
bootcmd:
...
runcmd:
  ...
  - |
      ...
      (
        ...
        -days 3650 \
        -nodes \
        -keyout "$DROPFOLDER/$PARENT_decr.pem" \
        -out "$DROPFOLDER/$PARENT.pem" \
        -subj "/CN=$PARENT" \
        -extensions v3_ca \
        -extensions v3_req \
        -config $DROPFOLDER/$PARENT.openssl.cnf

        echo "convert from pem to der"
        openssl x509 -in $DROPFOLDER/$PARENT.pem -out $DROPFOLDER/$PARENT.der -outform DER

        echo "printout the pem content"
        openssl x509 -noout -text -in $DROPFOLDER/$PARENT.pem

        echo "printout the der content"
        openssl x509 -noout -text -in $DROPFOLDER/$PARENT.der -inform der
...
Um die verfügbaren Standard Images aufzulisten kann der Befehl `az vm image list --all --location switzerlandwest -f ubuntu`  verwendet werden. Falls die verfügbaren locations notwendig sind, können diese mit `az account list-locations -o table` abgerufen werden Die Supported Platform können in der [IOT Edge Doku](https://learn.microsoft.com/en-us/azure/iot-edge/support?view=iotedge-1.4#tier-1) nachgeschlagen werden.
